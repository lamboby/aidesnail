...
import com.itrustoor.boby.aidesnail.R;
...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnuploadschool.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				dialog = ProgressDialog.show(MainActivity.this, "",
						"正在上传学校数据至云端服务器,请稍候");
				String createschool;
				createschool = "token=1&name="
						+ tschoolfullname.getText().toString().trim()
						+ "&short_name="
						+ tschoolshort.getText().toString().trim()
						+ "&province_id=" + provincecode + "&city_id="
						+ citycode + "&county_id=" + countycode + "&macs=";
				int inum = items.size();
				String macs = "";
				if (inum > 0) {
					macs = "[";
				...
				createschool = createschool + macs + "&principal="
						+ tschoolpic.getText().toString().trim() + "&phone="
						+ tschooltel.getText().toString().trim() + "&prcp_pwd="
						+ adminpassword.getText().toString().trim()
						+ "&staff_pwd="
						+ commonpassword.getText().toString().trim()
						+ "&agents_id=" + isp.getText().toString().trim()
						+ "&address=" + province.toString() + city.toString()
						+ county.toString()
						+ tschooladdress.getText().toString().trim();
				// + province+city+county+tschooladdress.getText();

				HttpUtil.sendHttpPostRequest(
						"/snail/createSchool",
						createschool, new HttpCallbackListener() {
							@Override
							public void onFinish(String response) {
								dialog.dismiss();
								Message message = new Message();
		...
		btncreatebarcode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					String contentString = "<fn>"
							+ tschoolfullname.getText().toString().trim()
							+ "</fn>" + "<sid>" + schoolid + "</sid>";
					int inum = items.size();
					contentString = contentString + "<mnum>" + items.size()
							+ "</mnum>";
					if (inum > 0) {
						for (int p = 0; p < items.size(); p++) {
							contentString = contentString + "<m" + p + ">"
									+ items.get(p).address + "</m" + p + ">";
						}
					}
					if (!contentString.equals("")) {
						// 根据字符串生成二维码图片并显示在界面上，第二个参数为图片的大小（）
						if (items.size() > 10) {
							Toast.makeText(MainActivity.this, "MAC数量不能超过10个",
						...
						Toast.makeText(MainActivity.this, "数据不能为空",
								Toast.LENGTH_SHORT).show();
					}
	...
	public void saveQR(String imgurl, String schoolid, String desc) {
		HttpUtil.sendHttpPostRequest(
				"/snail/saveQr", "qr_path="
						+ imgurl + "&sch_id=" + schoolid + "&desc=" + desc,
				new HttpCallbackListener() {
					@Override
					public void onFinish(String response) {
						Message message = new Message();
					...
					public void onError(Exception e) {
						...
						dialog.dismiss();
						new AlertDialog.Builder(MainActivity.this)
								.setTitle("错误").setMessage("保存二维码路径至云服务器失败!")
								.setPositiveButton("确定", null).show();
						Looper.loop();
					}
				});
	...
	public void initSpincounty(String pcode) {
		HttpUtil.sendHttpPostRequest(
				"/snail/county", "city_id="
						+ pcode, new HttpCallbackListener() {
					@Override
					public void onFinish(String response) {
						Message message = new Message();
						message.what = 2;
	...
	class SpinnerOnSelectedListener3 implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> adapterView, View view,
				int position, long id) {
			county = ((MyListItem) adapterView.getItemAtPosition(position))
					.getName();
			countycode = ((MyListItem) adapterView.getItemAtPosition(position))
					.getPcode();
		}
	...
	protected void saveBitmap() {
		...
		try {
			...
			File file = new File(appDir, tschoolshort.getText().toString()
					+ fileName);
			/*
			 * 保存图片 FileOutputStream stream = new FileOutputStream(file);
			 * imageviewbarcode.setDrawingCacheEnabled(true);
			 * imageviewbarcode.getDrawingCache().compress(CompressFormat.PNG,
			 * 100, stream); imageviewbarcode.setDrawingCacheEnabled(false);
			...
			final OSSData data = ossService.getOssData(bucket, countycode
					+ tschoolfullname.getText().toString() + "二维码.png");
			...
			data.uploadInBackground(new SaveCallback() {
				...
				public void onSuccess(String objectKey) {
					...
					// Looper.loop();
					saveQR(imgUrl, schoolid, city + county
							+ tschoolfullname.getText().toString());
				}
			...
			try {
				...
				if (resObject.getInt("Code") == 0) {
					...
					case 1:
						...
						MyAdapter myAdapter1 = new MyAdapter(MainActivity.this,
								list);
						citySpinner.setAdapter(myAdapter1);
						citySpinner
								.setOnItemSelectedListener(new SpinnerOnSelectedListener2());
						break;
					case 2:
						...
						MyAdapter myAdapter2 = new MyAdapter(MainActivity.this,
								list);
						...
						break;
					case 4:
						Toast.makeText(MainActivity.this, "二维码成功上传至云服务器", 0)
								.show();
						break;
					case 5:
						jsonArray = new JSONObject(response)
